...
{
    internal class LeftCavalryPotisionBattleLogic : PotisionBattleLogicBase
    {

        public LeftCavalryPotisionBattleLogic(Troop onTroop, CorpIdentDiv corpIdentId) : base(onTroop, corpIdentId)
        {
            this.onPotisioin = PotisioinDiv.leftCavalry;
        }
        ...
        public override BattleLogicResponse attack(CorpTroopBean onCorpTroopBean, CorpTroopBean opponentCorpTroopBean, CorpIdentDiv corpIdentId)
        {
            ...
            // 左翼歩兵ブロック判定
            if (blockCalc(onTroop, opponentCorpTroopBean.getTroopByPositionId((int)PotisioinDiv.rightInfantry), 30))
            {
                // 左翼歩兵にアタック
            ...
            // 左翼騎兵ブロック判定
            else if (blockCalc(onTroop, opponentCorpTroopBean.getTroopByPositionId((int)PotisioinDiv.rightCavalry), 20))
            {
                // 左翼騎兵にアタック
            ...
            battleLogicResponse.logTextList.Add(logTextIdent + "左翼騎兵 " + onTroop.general.name + "隊の攻撃：" + opponentCorpTroopBean.getTroopByPositionId((int)targetPos).general.name + "隊に：" + damage + "人の損害を与えた");
